# Рекламные плитки для витрины Chrome Web Store.
# ⚠ Магазин требует 24-битный PNG БЕЗ альфа-канала, поэтому логотип кладётся
# на сплошной фон, а не сохраняется с прозрачностью.
...
$logo = [System.Drawing.Image]::FromFile((Join-Path $root 'src\IPasswrd.App\Assets\ipasswrd_app_512.png'))

# фирменные цвета из App.axaml (тёмная тема)
...
function New-Promo([int]$w, [int]$h, [string]$name, [int]$logoSize, [single]$titleSize, [single]$subSize) {
    ...
    # тёплое пятно за логотипом - иначе плитка выглядит плоской заливкой
    ...
    $g.DrawString('Менеджер паролей', $fS, $bS, $tx, $ty + $th + $sh * 0.15)
    $g.DrawString('Сейф остаётся у вас', $fS, $bA, $tx, $ty + $th + $sh * 1.25)
